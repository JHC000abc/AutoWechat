...
import win32con
import win32gui
from utils.utils_md5 import Tools
...
class WxOperation:
    ...
    def _match_nickname(self, name) -> bool:
        """获取当前面板的好友昵称"""
    ...
    def _goto_chat_box(self, name: str) -> bool:
        """
        跳转到指定 name好友的聊天窗口。
    ...
    def __send_text(self, input_name, *msgs) -> None:
        ...
        for msg in msgs:
            ...
            try:
                ...
                print(f"未找到{input_name}输入框,跳过{input_name, msgs}")
                continue
            # 设置输入框为焦点并点击
    ...
    def __send_file(self, *file_paths) -> None:
        ...
        process = subprocess.Popen(args=args, startupinfo=startupinfo)
        process.wait()
        ...
        try:
            ...
            if not scroll:
                for name_node in contacts_management_window.ListControl().GetChildren():  # 获取当前页面的 列表 -> 子节点
                    nick_name = name_node.TextControl().Name.replace("\n", "")  # 用户名
                    remark_name = name_node.ButtonControl(foundIndex=2).Name.replace("\n",
                                                                                     "")  # 用户备注名，索引1会错位，索引2是备注名，索引3是标签名
                    tag_name = name_node.ButtonControl(foundIndex=3).Name.replace("\n",
                                                                                  "")  # 用户备注名，索引1会错位，索引2是备注名，索引3是标签名
                    user_info = {
                        "nick_name": nick_name,
                    ...
                    user_info_md5 = Tools.make_md5(user_info)
                    if user_info_md5 not in recode_set:
                        recode_set.add(user_info_md5)
    ...
    def send_msg(self, name, msgs, file_paths, add_remark_name=False) -> None:
        ...
        if not self._match_nickname(name=name):
            if not self._goto_chat_box(name=name):
